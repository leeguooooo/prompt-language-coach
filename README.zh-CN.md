...
配置存储于共享配置文件，并镜像到各平台路径：

- 共享：`~/.prompt-language-coach/language-coach.json`
- Codex 镜像：`~/.codex/language-coach.json`
- Claude Code 镜像：`~/.claude/language-coach.json`
- Cursor 镜像：`~/.cursor/language-coach.json`
...
插件经过设计，每轮新增的 token 消耗极低：

| 平台 | 静态规则位置 | 每轮 Hook 注入量 |
|---|---|---|
| Claude Code | `~/.claude/CLAUDE.md`（`@`-import 到 `~/.prompt-language-coach/claude-coaching.md`） | ~25 token（仅进度 note） |
| Codex | `~/.codex/AGENTS.md`（marker 块） | ~25 token（仅进度 note） |
| Cursor | 每次会话完整注入 | ~750 token（完整辅导 prompt） |

在 **Claude Code** 和 **Codex** 上，完整教学指令（~750 token）作为环境指令从平台配置文件一次性加载。`UserPromptSubmit` hook 每轮只注入一行简短的动态进度 note（如 `English: current estimate A2`），将每条 prompt 的额外开销控制在约 **25 token** —— 相比每轮注入完整 prompt，节省约 97%。
...
然后创建 `~/.prompt-language-coach/language-coach.json` 并填入你的配置。平台镜像不是必须的，因为运行时会先回退到共享配置。

### Cursor

克隆仓库并将其放置于 `~/.cursor/plugins/local/language-coach`，Cursor 会自动识别 `.cursor-plugin/plugin.json`。钩子会先读取 `~/.cursor/language-coach.json`，没有时再回退到共享配置。
...
最终配置存储在 `~/.prompt-language-coach/language-coach.json`，并镜像到平台路径：
